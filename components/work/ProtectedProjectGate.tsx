...
export function ProtectedProjectGate({
  slug,
  redirectPath = `/work/${slug}/full`,
  protectedLabel = "This case study is protected",
  submitLabel = "view full case study",
  successLabel = "opening case study",
}: {
  slug: string;
  redirectPath?: string;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("success");
      resetTimer.current = window.setTimeout(() => router.push(redirectPath), 300);
    } catch {
      setPassword("");
      setStatus("error");
      setErrorKey((value) => value + 1);
  ...
  return (
    <CaseStudyShell as="section" id="protected-access" className="protected-access" aria-labelledby="protected-access-label">
      <p id="protected-access-label" className="protected-access-label">{protectedLabel} <img src="/home-design/work-lock.svg" alt="" width="14" height="14" /></p>
      <form className="protected-gate-row" onSubmit={submit} noValidate>
        ...
        <motion.button className="portfolio-download-row protected-submit" type="submit" disabled={!password || status === "submitting" || status === "success"} aria-busy={status === "submitting"} data-status={status} data-cursor="Unlock" whileTap={reduceMotion ? undefined : { scale: .992 }}>
          <Magnetic className="protected-submit-inner" strength={3}><span>{status === "submitting" ? "checking" : status === "success" ? successLabel : submitLabel}</span><img src="/home-design/work-unlock.svg" alt="" width="24" height="24" /></Magnetic>
        </motion.button>
