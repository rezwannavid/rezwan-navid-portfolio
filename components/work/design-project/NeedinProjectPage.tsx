...
function LimeBlock({ x, y, width, height, className = "" }: { x: number; y: number; width: number; height: number; className?: string }) {
  return <div className={`needin-lime ${className}`.trim()} style={{ left: x, top: y, width, height }}>
    <ParallaxMedia className="needin-lime-depth" distance={0} xDistance={0} rotateDistance={0} reveal revealOffset={42}>
      <span className="needin-lime-fill" />
    </ParallaxMedia>
  </div>;
}
...
function MediaBlock({ src, alt, x, y, width, height, className = "", animatedSrc, animationClass = "" }: {
  ...
  return <div className={`needin-media ${className}`.trim()} style={{ left: x, top: y, width, height }}>
    <ParallaxMedia className="needin-media-depth" distance={0} xDistance={0} rotateDistance={0} reveal revealOffset={34}>
      <img src={src} alt={alt} />
      {animatedSrc ? <AnimatedMediaLayer src={animatedSrc} className={animationClass} /> : null}
    </ParallaxMedia>
  </div>;
}
    ...
    <main className="needin-page" style={{ height: DESIGN_HEIGHT }}>
      <article className="needin-canvas">
        <div className="needin-content">
          ...
          <Positioned className="needin-media-caption" x={96} y={8249} width={230}>Flow: Managing Schedule</Positioned>
          ...
          <LimeBlock x={96} y={9079} width={535} height={510} />
          <LimeBlock className="needin-cream" x={642} y={9079} width={460} height={273} />
          ...
          <LimeBlock className="needin-pale" x={237} y={9601} width={465} height={482} />
          <LimeBlock className="needin-green" x={712} y={9601} width={465} height={482} />
...
function RVisual({ className = "", src, alt = "", animatedSrc, animationClass = "" }: { className?: string; src?: string; alt?: string; animatedSrc?: string; animationClass?: string }) {
  return <div className={`needin-r-lime ${className}`}><ParallaxMedia className="needin-r-lime-depth" distance={0} xDistance={0} rotateDistance={0} reveal revealOffset={42}>{src ? <><img src={src} alt={alt} />{animatedSrc ? <AnimatedMediaLayer src={animatedSrc} className={animationClass} /> : null}</> : <span />}</ParallaxMedia></div>;
}
